...
//PA 2
//
//Note: working on making CSV file bigger
//	and seperating in to different files of code.'
//
...
void toFile (ofstream &file, string outFileName,string inFileName, string funName, int n, int result, double timeMicro);
void splitName(string &a,int next);
...
int main(int argc, char* argv[]){
	...
	string inFileName = argv[1], writeToFileName = "Max_data_result.txt";
	vector<int> ofInts;
	ifstream inFile;
	ofstream outFile;
	

	openFileCheck(inFileName, inFile, ofInts);
...
/*	for(int i = 0; i < atoi(argv[3]); i++)  {
	...
	for(int i = 0; i < tries; i++)  {
		...
		//double test = elapsed_microseconds.count();
		//cout << "int time " << test << endl;

		toFile(outFile, writeToFileName,inFileName,"maxSubSum1",
				ofInts.size(),max, elapsed_microseconds.count());
	}
	}*/
	for(int i = 0; i < atoi(argv[3]); i++)  {
		splitName(inFileName, i);
	...
	for(int i = 0; i < tries; i++)  {
		...
		toFile(outFile, writeToFileName,inFileName,"maxSubSum2",
				ofInts.size(),max, elapsed_microseconds.count());
	}
	}
	...
	for(int i = 0; i < tries; i++)  {
		...
		toFile(outFile, writeToFileName,inFileName,"maxSubSum3",
				ofInts.size(),max, elapsed_microseconds.count());
	}
	}
	...
	for(int i = 0; i < tries; i++)  {
		...
		toFile(outFile, writeToFileName,inFileName,"maxSubSum4",
				ofInts.size(),max, elapsed_microseconds.count());
	}
	}
...
void buildVector(ifstream &file, vector<int> &in)  {
	...
	//found what looks like a simple way of reading integers from a file.
	//think it acts like getline. 
	while(file >> item)  {
		in.push_back(item);
	}
}

//used to wite to file relivent info.
void toFile (ofstream &file, string outFileName, string inFileName, string funName, int n, int result, double timeMicro)  {

	file.open(outFileName, ofstream::app);

	file << inFileName << "," << funName << "," << n << "," 
			<< result << "," << timeMicro <<"\n";
...
//changes file name so that i can run 10 files per run
void splitName(string &a,int next)  {

	int end = a.size() - 1;

	// gets us to the number before .txt ie input_8_[].txt
	end = end - 4;
	...
	string bob = to_string(next) + ".txt";

	a.replace(end,end,bob);
}
