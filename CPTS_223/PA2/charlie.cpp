...
//PA 2
//
//Note: charlie is to write to file in csv format 
//to later be converted into exeal so to plot data.
//
//
...
int main(int argc, char* argv[]){
	...
	for(int i = 0; i < tries; i++)  {
		...
		toFile(outFile, writeToFileName,argv[1],"maxSubSum1",
	...
	for(int i = 0; i < tries; i++)  {
		...
		toFile(outFile, writeToFileName,argv[1],"maxSubSum2",
				ofInts.size(),max, elapsed_microseconds.count());
	}
	...
	for(int i = 0; i < tries; i++)  {
		...
		toFile(outFile, writeToFileName,argv[1],"maxSubSum3",
				ofInts.size(),max, elapsed_microseconds.count());
	}
	...
	for(int i = 0; i < tries; i++)  {
		...
		toFile(outFile, writeToFileName,argv[1],"maxSubSum4",
				ofInts.size(),max, elapsed_microseconds.count());
	}
